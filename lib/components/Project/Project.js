  ...
  component({
    ...
    * onUpdate (prev, {props, state, actions, context}) {
    ...
    controller: {
      ...
      * save({context,actions}, name, contents) {
        console.log("NAME:", name)
        
        // don't invalidate appBundle cache if only package.json modified
        if (name != `${context.projectId}/package.json`)
    ...
    reducer: {
      ...
      setPackageJSONError: (state, val) => {
        console.log("Setting packageJSONERROR to", val)
        return {packageJSONError: val}
      },
        ...
        >
          <Block id="header" borderTop borderLeft borderRight borderColor="#aaa">
            <Flex wide align='space-between center'>
              <h2 style={{margin: 0, padding:"3px"}}>
                boxspring&nbsp;/&nbsp;
                <span style={{color: "#0090ee"}}>{context.projectId}</span>
              </h2>
              { state.packageJSONError && 
                <h3 fontFamily='ornate' style={{textAlign: "center", backgroundColor: "red", color: "white", borderTop: "1px solid #aaa", borderBottom: "1px solid #aaa", marginTop: 0, marginBottom: 0}}>
                  Build aborted: package.json is not valid JSON
                </h3>
              }
          ...
          >
            ...
            <PreviewManager
              ...
              setPackageJSONError={actions.setPackageJSONError}
            />
          </Flex>
