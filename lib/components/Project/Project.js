    ...
    * onCreate ({props, context, actions, state}) {
    ...
    controller: {
      ...
      * closeTab({actions, state, props}, name) {
        ...
        yield actions.removeFileFromOpenTabs(name)
        console.log(state)
        
        let nextActiveFile = (openTabs.length) ? openTabs[0].name : null
        actions.setActiveFile(nextActiveFile)
    ...
    reducer: {
      ...
      removeFileFromOpenTabs(state, name) {
        let newOpenTabs = state.openTabs.filter(tab => tab != name)
        console.log("RemoveFileFromOpenTabs", newOpenTabs)
        return { ...state, openTabs: newOpenTabs }
      }
