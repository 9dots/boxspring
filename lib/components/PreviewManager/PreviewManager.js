  ...
  * onCreate({state, actions}) {
  ...
  controller: {
    * buildAndRun({props, state, actions, context}, server) {
      ...
      yield props.saveOpenFilesToGCS()

      // validate packageJSON (fetch, then parse)
      let pjText = yield props.getFileContentsFromName(`${context.projectId}/package.json`)

      try {
        JSON.parse(pjText)
      } catch (e) {
        // don't build project if package.json is invalid
        console.log('Package.json is invalid. Aborting build')
        yield props.setPackageJSONError(true)
        console.timeEnd('buildAndRun')
        yield actions.setBuildInProgress(false)
        return
      }

      yield props.setPackageJSONError(false)
    ...
    return props.collapsed ? <span /> : (
    	...
    	>
          ...
          <Flex align='flex-end center' h="35px">
            {false && <GameButton disabled={buildInProgress} onClick={actions.buildAndRun('local')} bg={buildInProgress ? "gray" : "green" }  px='5px' py='3px' h="25px" fs="14px" color={buildInProgress ? "#ccc" : "black" } mr="4px">
            ...
            <GameButton disabled={buildInProgress} onClick={actions.buildAndRun} bg={buildInProgress ? "gray" : "blue" }  px='5px' py='3px' h="25px" fs="14px" color={buildInProgress ? "#ccc" : "black" }>
                {buildInProgress || <Icon fs='inherit' bolder name='build' mr='3px'/>}
