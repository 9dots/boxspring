  ...
  * onCreate({state, actions}) {
  ...
  controller: {
    ...
    * buildAndRun({props, state, actions, context}, server) {
      ...
      let bundleURL = yield actions.buildBundle(server)
      ...
      console.log("Build time", (endTime - startTime)/1000.0, "seconds")
    ...
    * buildBundle({context}, server) {
      let url
      if (server === "local")
        url = `http://localhost:3000/buildProject?`
      else
        url = `https://us-central1-boxspring-81172.cloudfunctions.net/build?`
    ...
    return props.collapsed ? <span /> : (
    	...
    	>
          ...
          <Flex align='flex-end center' h="35px">
            <GameButton disabled={buildInProgress} onClick={actions.buildAndRun('local')} bg={buildInProgress ? "gray" : "blue" }  px='5px' py='3px' h="25px" fs="14px" color={buildInProgress ? "#ccc" : "black" } mr="4px">
        ...
        <Block id="preview-pane" backgroundColor="white" h='95%' w='100%'>
          <Sandbox userScript={state.bundle} userHTML={state.html}/>
        </Block>
      </Block>
    )
  }
