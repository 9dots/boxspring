  ...
  * onCreate({state, props, actions}) {
  ...
  controller: {
    ...
    * buildAndRunApp({props, state, actions, context}, server) {
      ...
      let indexContent = yield props.getFileContentsFromName(`${context.projectId}/index.js`)
      let {ok, output} = yield context.getAppBundleFromServer(context.projectId, indexContent, "")  //need to add state for app bundle cache name
      let appBundle = output
      if(!ok){
