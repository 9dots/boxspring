  ...
  * onCreate({state, props, actions}) {
  ...
  controller: {
    ...
    * buildAndRunApp({props, state, actions, context}, server) {
      ...
      let css = yield props.getFileContentsFromName(`${context.projectId}/index.css`)
      let appBundle = ""

      let failedUpload = yield actions.uploadSourceJSFiles()

      if(failedUpload){
        console.log("Failed while uploading files to server")
        yield actions.setErrorMsg("Failed to upload source files to server...")
  ...
  render ({state, props, actions}) {
    let {buildInProgress, depsBundle, appBundle, appHTML, errorMsg, appCSS} = state
