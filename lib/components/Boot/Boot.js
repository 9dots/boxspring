...
import gcs from 'client/gcsClient'
...
export default component({
  ...
  getContext ({state, actions}) {
    return {
      ...
      url: state.url,
      uid: state.uid,
      serverUid: state.serverUid,
      welcome: state.welcome,
      uiTheme: theme,
      BUCKET_NAME: "boxspring-81172.appspot.com",
  ...
  initialState: {
    ...
    projectId: '',                    //name of the currently selected project
    uid: null,                        //uid from firebase, used for auth
    serverUid: null,                        //uid from firebase, used for auth
    loadingAuth:true,
    welcome: null,
    title: 'Boxspring',
    modal: null
  ...
  render ({props, state, actions}) {
    if(state.loadingAuth){                    //loadingAuth becomes true after the firebaseLogin mw has determined whether there's a user
