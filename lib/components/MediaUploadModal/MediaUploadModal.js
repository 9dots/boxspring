...
export default component({
  ...
  render ({props, actions, state}) {
	  ...
	  const body =
	  (<Block column m={0} >
		    <Form m={0} p={0} id='submit-form-media' onSubmit={actions.submit(fileSrc, fileName)} validate={validate}>
		      {
		      	textarea 
		      		? <MyTextArea
		      				flex
  ...
  controller: {
    ...
    * readFile({state, actions, props, context}) {
      var inputs = document.getElementsByClassName('vui-input')
  	  ...
  	  if(!mediaInput){
  	  	console.log("Couldn't find a file input")
  	  	return
  	  }

	  var input = mediaInput.files
