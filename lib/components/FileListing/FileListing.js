 ...
 * <File Listing/>
 */
var DISPLAY_NAME_CUTOFF = 13

export default wrap(CSSContainer, {})(component({
  render ({props, actions, context}) {
    let file = props.file
    var fixedDisplayName = file.displayName
    if(props.hovering && file.displayName.length > DISPLAY_NAME_CUTOFF){
      fixedDisplayName = file.displayName.substr(0,DISPLAY_NAME_CUTOFF-3) + "..."
    }
    return (
      <Flex align="space-between center" bg={props.bgColor} mb={props.mb} onClick={props.downloadFile(file)}>
