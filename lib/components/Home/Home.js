    ...
    return (
      <div>
        <Block fontFamily='Roboto' id="container" ml="auto"
          ...
          <Block id="header" borderTop borderLeft borderRight borderBottom
                 borderColor="#aaa" textAlign="right">
            <Flex align='space-between center'>
            <h2 flex style={{margin: 0, padding:"3px 3px 3px 3px",}}>boxspring: Welcome {context.welcome}</h2>
            <GameButton float="right" bg='red'  px='12px' py='6px' h="34px" fs="15px" color="black" onClick={actions.handleSignOut()}>Sign Out</GameButton>
            </Flex>
          </Block>
  	      <ProjectCreator {...props} />
          <Block id="header" textAlign="center">Projects</Block>
  	      <ProjectLoader {...props} />
          <Footer/>
        </Block>
