...
export default component({
  ...
  * onCreate({state, props, actions}) {
  ...
  controller: {
    ...
    * testFileReader({state, actions, props, context}, input, input2) {
    ...
    * uploadFile({actions, props}){
        yield actions.createFile({filename:document.getElementById('testImage').label, fileContent:document.getElementById('testImage').src})
    },
  ...
  render ({props, actions, state, context}) {
    ...
    return !fileTree ? <ul>Loading...</ul> : (
      <Block
        ...
        <input type='file' id="imgInp" onChange={actions.testFileReader(this)}>OI</input>
