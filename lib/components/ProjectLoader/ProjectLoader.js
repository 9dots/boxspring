...
export default component({
  ...
  controller: {
    ...
    * renameProject ({context, props, state, actions}, projectKey, projectName) {
      if(!projectName)
        projectName = "NAME_NOT_FOUND"
      var response = prompt("Enter new name for Project \"" + projectName + "\"", "Awesome New Project Name")
      if(response){
        yield context.firebaseSet(`projects/${projectKey}/projectId`, response)
        yield context.firebaseSet(`users/${props.uid}/projectList/${projectKey}/projectName`, response)
      } else {
