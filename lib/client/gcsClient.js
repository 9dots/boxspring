...
function * getAppBundleFromServer(projectId, appContent="", bundleName=DEFAULT_APP_BUNDLE_NAME){
	console.log("GCS Getting App Bundle on Server")
	if(bundleName == DEFAULT_APP_BUNDLE_NAME){
		...
		if(!ok)
			return {ok:false, output:"Failed to create app bundle on server"}
		bundleName = bundle
	}
